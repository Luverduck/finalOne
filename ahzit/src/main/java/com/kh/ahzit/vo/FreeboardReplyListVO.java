...
@Data
public class FreeboardReplyListVO {
	...
	public int blockEnd() {
		// ex) 한 페이지에 10개의 페이지 블럭을 표시할 때 전체 페이지의 페이지 블럭 총 갯수가 63개이면 총 댓글 갯수는 630개
	...
	public int blockCount() {
		// ex) 총 댓글의 갯수가 630개일 때 페이지 블럭의 총 갯수 (630 + (10 - 1)) / 10 = 63개
		return (total + (cntRow - 1)) / cntRow;
	}
	
	// - 전체 페이지의 마지막 페이지 블럭 번호 반환
	public int blockLast() {
		// ex) 총 댓글의 갯수가 630개일 때 마지막 페이지 블럭 번호 (630 + (10 - 1)) / 10 = 63이므로
