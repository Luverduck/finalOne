...
- 구현 기능 회의
  - FAQ  게시판 추가 (답변 없고 관리자가 작성/삭제하는 형태) + 검색 가능하도록
  - QnA 게시판 질의 형식(모든 사용자가 확인 가능)
...
- 1차 역할 분담 (임시)
  - 회원 기능 - 의중
    - 회원가입(비동기) / 로그인, 로그아웃 / 회원 정보 수정 / 회원 탈퇴 / 회원 프로필 첨부파일
    - 이메일 인증 CD

  - 게시판 - 승리
    - 공지 게시판 CRUD / 첨부파일
    - QnA 게시판 CRUD / 첨부파일
    - 1:1 문의 게시판 CRUD / 첨부파일
    - 1:1 문의 게시판 댓글 CRUD

  - 소모임 (1) - 소정 / 지안
    - 소모임 CRUD (개설 / 삭제 등..) / 소모임 프로필 첨부파일
    - 소모임 회원 CRUD / 소모임 회원 프로필 첨부파일

  - 소모임 (2) - 현용
    - 소모임 게시판 CRUD / 소모임 게시판 이미지 첨부파일
    - 소모임 게시판 댓글 CRUD
    - 자유 게시판 CRUD / 첨부파일
    - 자유 게시판 댓글 CRUD

  - 관리자 기능 - (미정)
    - 회원 목록 보기 / 회원 상세 보기 / 회원 블라인드
    - 회원 1:1 메세지
    - 소모임 비활성화 / 소모임 해체
...
- 구현 기능 정리
  - 채팅, 결제 기능 제외 / 로그인, 게시판 등 기본적인 CRUD 기능 심화 구현 / 구현 후 시간 상황 보고 제외 했던 기능 구현
  - 소모임 내 게시판, 댓글 작성 시 각 회원에게 활동 점수 부여 후 소모임 내 전체 회원의 활동 점수를 합하여 소모임 활동 점수 구현 (보류)
  - 소모임 내의 회원 등급은 개설자, 일반 회원 / 회원 내의 등급은 관리자, 일반 회원 
  - 신고 기능 : 소모임 자체를 신고하여 홈페이지 관리자가 확인하여 조치를 취하는 방식
  - 소모임 내 게시판 이 외 전체 사용자가 이용 할 수 있는 전체 게시판 구현 (자유게시판)
  - 소모임 게시글 등록 시 일정 등록 추가 가능, 사이드 바에 소모임 내의 일정을 볼 수 있는 캘린더 버튼 추가하여 일정 관리
...
- 관리자 기능 발표 및 토론
  - 소모임 관리자(개설자)와 관리자(Admin)의 권한을 어떻게 나눌지 토론
    - 소모임 관리자(개설자)의 관리 페이지는 소모임 내 설정 버튼을 눌러서 관리하는 방식
    - 관리자(Admin)의 관리 페이지는 관리자 계정으로 로그인 시 바로 관리자 페이지의 메인으로 redirect 하는 방식
- 멤버십 결제 기능 발표 및 토론
  - 멤버십의 혜택은 무엇인지
    - 멤버십보다는 소모임 총원을 확대하는 인원 증가권 형태로 구현
- 기능 정의 시트 정리 및 확정
- 다음 단계의 역할 분담
  - ERD : 의중, 현용
  - UI : 소정, 승리
...
- 기능 정의 
  - 메인 페이지
    ...
    - 공지 게시판 CRUD
    - 드롭다운 형태로 내 정보탭 구현(메뉴 : 마이페이지 / 로그아웃)
    - 찾기 메뉴를 통해 모든 카테고리의 소모임들을 볼 수 있으며 해당 카테고리의 소모임들 표시
    - 메인 페이지에서 전체 채팅 구현
    - 내가 가입한 소모임 보기
    - 소모임 추천 (추천 기준 : 랜덤)
    - 신규 개설된 소모임 표시 (추천 기준 : 랜덤)
  - 소모임 개설 및 관리
    - 소모임 소개 페이지(대문)
    - 소모임 개설 / 정보 수정
    - 소모임 가입 / 탈퇴
  - 소모임 관리 및 권한 부여
    - 소모임 해체는 소모임 개설자만 가능
    - 소모임 개설자의 멤버 CRUD 가능
    - 소모임 개설자는 소모임에 가입된 다른 멤버에게 개설자 권한 위임 가능  
  - 소모임 기능
    - 게시글 CRUD (일정 추가는 게시글 작성 형태로 할 수 있음)
    - 사진첩 CD
    - 댓글 CRUD
  - 회원 관리
    - 회원 가입
  ...
  - 기능 정의 시트에서 관리자 기능 및 멤버십 결제에 대해 생각해오기
